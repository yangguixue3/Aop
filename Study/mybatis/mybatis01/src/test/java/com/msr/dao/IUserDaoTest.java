...
public class IUserDaoTest {
    ...
    @Test
    public void testFindAllByQueryVO() throws IOException {
        // 5.使用代理对象来执行函数
        ...
        List<User> userList = mapper.findAllByQueryVO(queryVO);

        userList.stream().forEach(user1 -> System.out.println("user1 = " + user1));
    }
    ...
    public void testFindUserByCondition() throws IOException {
        ...
        user.setUserName("%王%");
        // user.setUserSex("女");
        List<User> userList = mapper.findAllByCondition(user);
    ...
    public void testFindAllByIds() throws IOException {
        ...
        queryVO.setIds(ids);
        List<User> userList = mapper.findAllByIds(queryVO);
